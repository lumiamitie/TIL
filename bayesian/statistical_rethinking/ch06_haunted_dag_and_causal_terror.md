...
다음 DAG는 관심 대상 X, 결과 변수 Y, 관측된 변수 A,B,C, 그리고 관측되지 않은 변수 U로 구성되어 있다.
...
우리가 알고자 하는 것은 X의 Y에 대한 인과적인 효과다. 그렇다면 정확한 효과 측정을 위해서는 어떤 변수를 모형에 추가해야 할까? 
백도어 경로를 살펴보자. 

1. X ← U ← A → C → Y
    - Collider가 없기 때문에 **열려있는 경로다**
    - 변수 중 하나를 통제해야 하는데, U는 관측할 수 없기 때문에 통제할 수 없다
    - **A 또는 C를 통제하면 백도어를 닫는다**
2. X ← U → B ← C → Y
    - Collider를 포함하고 있기 때문에 **이미 닫힌 경로다**
    - 그런데 **B를 통제하는 순간 경로가 열리고 교란** 효과를 일으키게 된다

`dagitty` 라이브러리의 `adjustmentSets` 함수를 통해 어떤 변수를 통제해야 하는지 쉽게 확인할 수 있다.
...
adjustmentSets(dag01, exposure = 'X', outcome = 'Y')
# { C }
# { A }
```
...
5장에서 다루었던 와플집과 이혼율 문제로 다시 돌아와보자. DAG를 만들고, 최소한의 공변량을 찾아낸 다음 테스트 가능한 DAG의 구현을 만들어 낼 것이다. 
이러한 작업을 통해 DAG가 정말 우리가 확보한 증거에 부합하는지 확인할 수 있다. 
데이터만으로는 DAG가 맞는지 확인할 수 없지만, 적어도 DAG가 틀렸는지는 알 수 있다. 그래프를 만들어보자.
...
W와 D 사이에는 세 개의 열린 백도어 경로가 존재한다. 그런데 모든 백도어가 S를 지나가기 때문에 S를 통제하면 모든 열린 백도어를 닫을 수 있다. 
R에서 확인해보면 결과는 다음과 같다. S를 통제하거나, A와 M을 통제해야 한다는 결과를 얻을 수 있다. 
...
adjustmentSets(dag02, exposure = 'W', outcome = 'D')
# { A, M }
# { S }
```

이제 DAG가 현재 데이터에 부합하는지 확인해보려면 테스트 가능한 DAG 구현이 필요하다. 그 중 하나는 이전 장에서 다루었던 조건부 독립을 활용하는 것이다. 
DAG 상에서 조건부 독립이어야 하는 변수들의 조합을 찾아보고, 실제로 그러한지 비교해본다. 

```r
impliedConditionalIndependencies(dag02)
# A _||_ W | S
# D _||_ S | A, M, W
# M _||_ W | S
```
